...
class BlimpRecordModifier extends \Bolt\Storage\RecordModifier {
    protected $app;
    ...
    public function handleSaveRequest(array $formValues, array $contenttype, $id, $new, $returnTo, $editReferrer) {
        if (empty($contenttype['blimp_mode']) || $contenttype['blimp_mode'] !== 'remote') {
            if(!empty($contenttype['blimp_mode']) && $contenttype['blimp_mode'] === 'sync') {
                $formValues['status'] = 'draft';
            }

            return parent::handleSaveRequest($formValues, $contenttype, $id, $new, $returnTo, $editReferrer);
        }
        ...
        if ($id) {
            $content = $this->app['blimp_client.storage']->getContent($contenttypeslug, ['id' => $id]);
            $oldStatus = $content['status'];
        } else {
            $content = $this->app['blimp_client.storage']->getContentObject($contenttypeslug);
            $oldStatus = '';
        }
    ...
    private function saveContentRecord(Content $content, array $contenttype, $new, $comment, $returnTo, $editReferrer) {
        // Save the record
        $id = $this->app['blimp_client.storage']->saveContent($content, $comment);
    ...
    private function createJsonUpdate($contenttype, $id, $flush) {
        ...
        $content = $this->app['blimp_client.storage']->getContent($contenttype['slug'], ['id' => $id]);
    ...
    public function handleEditRequest($content, array $contenttype, $id, $new, $duplicate) {
        if (empty($contenttype['blimp_mode']) || $contenttype['blimp_mode'] !== 'remote') {
            return parent::handleEditRequest($content, $contenttype, $id, $new, $duplicate);
        }
