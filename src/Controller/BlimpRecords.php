...
class BlimpRecords extends Records
{
  ...
  public function getContent($textquery, $parameters = '', &$pager = [], $whereparameters = [])
  {
    ...
    if (!empty($contenttype['blimp_mode']) && $contenttype['blimp_mode'] === 'sync') {
      $local = $this->app['storage']->getContent($textquery);
      $remote = $this->app['blimp_client.storage']->getContent($textquery);

      foreach ($local as $li => $l) {
        if ($l->values['status'] == 'draft') {
          continue;
        }

        $f = false;
        foreach ($remote as $ri => $r) {
          if ($l->values["blimp_id"] == $r->values["id"]) {
            $f = true;
            break;
          }
        }
        ...
        if (!$f) {
          $this->app['storage']->deleteContent($textquery, $l->id);
        }
      }
      ...
      foreach ($remote as $ri => $r) {
        ...
        if (!$f) {
          $r->id = null;
          $r->values["blimp_id"] = $r->values["id"];
          $r->values["id"] = null;
          $r->values["status"] = "published";
          $this->app['storage']->saveContent($r, 'sync');
        }
      }
    }
